...
const Product = (props) => {
    const { img, name, seller, price, stock, ratingsCount, category, id } = props.product;
    return (
        <div className='w-full grid grid-cols-2 lg:grid-cols-3 border-b border-gray-300 p-4 pr-0 lg:pr-4 lg:pl-0 gap-6'>
            <div className="col-span-1">
                <img src={img} alt={category} />
            </div>
            <div className="col-span-1 lg:col-span-2 text-left grid">
                <Link to={`${id}`}>
                    <h2 className='text-blue-600 text-base lg:text-2xl hover:underline'>{name}</h2>
                </Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-16">
                    <div className="flex flex-col justify-center gap-1 lg:gap-4 mr-4 lg:mr-0">
                        ...
                        <button id='addToCart-btn' className='bg-amber-400 hover:bg-amber-500 rounded-md lg:px-16 py-1' onClick={(e) => { props.addToCart(props.product) }}><FontAwesomeIcon icon={faShoppingCart}></FontAwesomeIcon> Add to cart</button>
                    </div>
